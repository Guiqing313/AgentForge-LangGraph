...
| Chroma 默认 EF 不可用 | 用默认 EF 建集合时尝试下载 ONNX MiniLM 到 `<home>\.cache\chroma` → PermissionError | A1 必须自定义 EF；CI 必须注入 fake EF |
...
| OLLAMA_MODELS 未设置 | 用户/系统环境变量为空；模型 manifest 实际在 `<ollama-models-dir>` | G0/A0：启动前显式设置 |
| 模型存在 | `<ollama-models-dir>\manifests\...\qwen2.5\7b`、`...\nomic-embed-text\latest` | 启动后 `ollama list` 复核 |
...
# Ollama（正常终端）
$env:OLLAMA_MODELS="<ollama-models-dir>"
ollama serve
ollama list

# embedding 预检（pytorch_env）
<embedding-env>\python.exe -c "from FlagEmbedding import BGEM3FlagModel; m=BGEM3FlagModel(r'<local-smartkb>/models/bge-m3', use_fp16=True); print(m.encode(['RAG'], return_dense=True)['dense_vecs'].shape)"
```